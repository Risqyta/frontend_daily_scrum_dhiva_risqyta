        ...
        <form ref="form">
         <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input v-model="activity_yesterday" type="text" name="activity_yesterday" class="form-control" id="activity_yesterday" placeholder="Activity Yesterday" >
          </div>
          <div class="form-group">
            <label for="activity_today" class="col-form-label">Activity Today</label>
            <input v-model="activity_today" type="text" name="activity_today" class="form-control" id="activity_today" placeholder="Activity Today" >
          </div>
          <div class="form-group">
            <label for="problem_yesterday" class="col-form-label"> Problem Yesterday </label>
            <input v-model="problem_yesterday" type="text" name="problem_yesterday" class="form-control" id="problem_yesterday" placeholder="Problem Yesterday" >
          </div>
          <div class="form-group">
            <label for="solution" class="col-form-label">Solution</label>
            <input v-model="solution" type="text" name="solution" class="form-control" id="solution" placeholder="Solution" >
          </div>
        </form>
    </b-modal>
  </div>
...
module.exports = {
  ...
  methods: {
    getData: function() {
       ...
       this.axios.get("/login", conf)
        .then(response => {
          this.id_users = response.data.user.id;
          this.axios.get("/input_daily_scrum/" + this.perPage + "/" + offset + "/" + response.data.user.id, conf)
            .then(response => {
    ...
    Save : function(){
      ...
      if(this.action === "insert"){
        ...
        this.axios.post("/input_daily_scrum", form, conf)
        .then(response => {
   ...
   Drop : function(id){
      ...
      if(confirm('Apakah anda yakin ingin menghapus data ini?')){
        this.$bvToast.show("loadingToast");
        this.axios.delete("/input_daily_scrum/" + id, conf)
        .then(response => {
            this.getData();
            this.$bvToast.hide("loadingToast");
            this.message = response.data.message;
